...
/* PDF \u2014 mehrseitiges Querformat im App-/LitM-Look */
var PDF_LAYOUT={pageW:297,pageH:210,mx:14,my:14};
var PDF_COLORS={
...
function pdfMightColor(type){ return type==='Adventure'?PDF_COLORS.adventure : type==='Greatness'?PDF_COLORS.greatness : PDF_COLORS.origin; }

// Cormorant Garamond (App-Schrift) erst beim Export lazy nachladen
var _pdfFontsPromise=null;
function loadPdfFonts(){
  if(window.MISTHELD_FONTS) return Promise.resolve(true);
  if(_pdfFontsPromise) return _pdfFontsPromise;
  _pdfFontsPromise=new Promise(function(res){
    var s=document.createElement('script');
    var ver=((document.querySelector('.app-version')||{}).textContent||'').trim();
    s.src='fonts/cormorant.js'+(ver?('?v='+encodeURIComponent(ver)):'');
    s.onload=function(){res(!!window.MISTHELD_FONTS);};
...
function pdfRegisterFonts(doc){
  var F=window.MISTHELD_FONTS; if(!F) return false;
  try{
...
function pdfPaper(doc){ var L=PDF_LAYOUT,C=PDF_COLORS; doc.setFillColor.apply(doc,C.paper); doc.rect(0,0,L.pageW,L.pageH,'F'); }
function pdfMiniLabel(doc,FF,label,x,y){ doc.setFont(FF,'bold'); doc.setFontSize(7); doc.setTextColor.apply(doc,PDF_COLORS.gold); doc.text(label,x,y); return y+4.6; }
// Gold-Trenner mit kleinem Rauten-Ornament (statt \u2756-Glyph)
function pdfDivider(doc,cx,y,halfW){
...
function pdfThemeCard(doc,FF,theme,x,y,w,h){
  ...
  theme.powerTags.forEach(function(t){ var ls=doc.splitTextToSize(capitalizeFirst(t.text),w-12); doc.text(ls,x+6,cy); cy+=ls.length*5; });
  ...
  // Fu\u00dfband (Quest, Might-Farbe) am Kartenboden
...
function pdfHeroPage(doc,FF){
  ...
  // Platzhalter unten: Backpack / Fellowship / Notizen (leer, ausf\u00fcllbar)
  var bh=46, by=L.pageH-L.my-bh, bw=(w-2*6)/3;
  [STRINGS.pdf.backpack,STRINGS.pdf.fellowship,STRINGS.pdf.notes].forEach(function(lab,i){
    var bx=x+i*(bw+6);
...
async function generatePDF() {
  ...
  try {
    ...
    // Seite 1: Held
    pdfHeroPage(doc,FF);
    // Seite 2: 4 Themes nebeneinander
    ...
    prop.themes.forEach(function(_,i){ pdfThemeCard(doc,FF,getDisplayTheme(i),L.mx+i*(cW+gap),cardY,cW,cH); });
